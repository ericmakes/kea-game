...
**MORE EVIDENCE, session 33 (2026-09-07), from four directions in one session.**
  1. `17_flight` came back ssim 0.9610 then 0.9993 against the same baseline on an IDENTICAL tree.
  2. `03_kea_plate` came back 0.9648 then 0.9997 on the STASHED pre-change tree — which is how the
     pre-pin gate's two flagged frames were proved not to be the change.
  3. The eleven-run consensus for the six new vantages split by CONTENT, exactly as the diagnosis
     predicts: `37_river_bridge` and `39_river_walk` were IDENTICAL across all eleven processes
     (water, timber and shingle, almost no grass), while `38`, `41` and `42` all came back bimodal
     (tussock in frame). `40_station_yards` was neither — a smooth 1649..5504 spread, because its
     subject is a moving mob of sheep rather than a wind phase.
  4. `41_station_shed` is a 6/5 split at N=11 — a fair coin, the second frame to land there after
     `03_kea_plate`. **Until this is fixed, 03_kea_plate and 41_station_shed will each flag on
     roughly half of all sweeps, and that is not a regression.** Two modes ~8,600 px apart.
The diagnosis has now held from every direction it has been tested from. It is the highest-value

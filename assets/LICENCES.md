...
## THE PLACEHOLDER BOX  (REPLAT P6A — the model-swap seam, 2026-09-05)

<!-- ASSET file=models/placeholder_box.glb md5=c89eec5ae86e248d2e8e8e105122ccb0 attrib=none author="kea-gauntlet" title="P6A placeholder box" licence="CC0-1.0" -->

| field | value |
|---|---|
| file | `models/placeholder_box.glb` |
| what it is | a single flat-shaded box, 1688 bytes, one PBR material, no textures |
| author | **generated by this project** — `gauntlet/verify/mkplaceholder.mjs` |
| source | none. It is not a download. |
| licence | **CC0 1.0** — public domain dedication, no attribution required |
| md5 | `c89eec5ae86e248d2e8e8e105122ccb0` |

**IT IS NOT A DOWNLOAD, AND THAT IS THE POINT.** P6A.md forbids the piece from sourcing, adapting or
importing any real model, and it also requires an end-to-end swap proof — which needs a GLB. So the
asset is GENERATED, from twenty lines of arithmetic in `gauntlet/verify/mkplaceholder.mjs`, and its
provenance is stronger than any download's: re-run the script and md5 the output. There is no
upstream to verify against and none is claimed. The verification protocol at the top of this file
still applies in the form that makes sense here — **the md5 above is the md5 of the file the script
writes**, and the battery holds the bytes on disk to it exactly as it does for the Poly Haven sets.

**WHY IT IS 190 x 98 x 60 MODEL UNITS.** They are the bench's own proportions in centimetres, so the
loader has to divide by about a hundred to land it in game metres — a placeholder already at game
scale would let a broken normalisation look perfectly correct, which is the one thing a proof must
not allow. Its origin sits at the foot rather than the centre for the same reason: `fit.ground` has
to be exercised by something, and a box already standing on its own origin would prove nothing.

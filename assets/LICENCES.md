...
### THE MACHINE-PAINTED ARRIVAL — `models/kea_astra.glb`  (REPLAT P5F, 2026-09-05)

**THIS ROW EXISTS BEFORE ANYTHING WAS DONE WITH THE FILE, and that is the point.** The battery went
red the moment the bytes landed — `every model file in the tree has a licence row — UNLICENSED IN
TREE: kea_astra.glb` — and stayed red until this row was written. A file cannot sit in `assets/`
earning its keep while its provenance is a to-do.

`kea.glb`, produced by **OpenAI GPT-6 Astra** from `docs/ASTRA_BRIEF.md` and the reference plates,
landed here as a third artifact. It is **not** an edit of ours: the geometry is a new
wings-spread mesh wearing `rockatoo.glb`'s skeleton, with every bone systematically renamed
(`kea_` prefix, `_bone_NN` suffix) and the 60-joint cockatoo crest restored. It is a derivative of
`rockatoo.glb` all the same — same 161-joint skin, same `Animation_01` with its 311 channels — so
it carries CC-BY 4.0 and Macauley.B's credit, exactly as `kea_base` and `kea_bill` do.

| field | value |
|---|---|
| derived from | `models/rockatoo.glb` (Macauley.B, CC-BY 4.0) — the skeleton and the animation clip |
| licence | **CC-BY 4.0, unchanged** — a derivative of a CC-BY work carries the same terms |
| produced by | **OpenAI GPT-6 Astra**, from `docs/ASTRA_BRIEF.md` + the kea plates in `gauntlet/reference/board/` |
| changes | new 20,000-triangle body/wing/tail mesh on the inherited skin; all 161 bones renamed; crest joints and geometry present again; new 4096x2048 painted baseColour and 4K feather normal; alpha-MASK material |
| geometry (file) | **20,000 triangles / 11,082 vertices**, one primitive `Kea_Body_Wings_Tail` |
| skin | 161 joints — `kea_<Original>_bone_NN` |
| animations | 1 clip, `Animation_01`, 311 channels |
| material | `Kea_Olive_Feathers_Orange_Underwings` — alphaMode MASK, cutoff 0.45, doubleSided, metallic 0, roughness 0.84, normalScale 0.6 |
| textures | 2 PNG, both **4096x2048**: `Kea_BaseColour` 11,274,987 B (RGBA), `Kea_Feather_Normal` 2,913,463 B (RGB) |
| generator string | `Kea reference adaptation / GLB 2.0` — not Blender |
| file | `models/kea_astra.glb`, 18959236 bytes |
| md5 | `fe502de29c851dbfd0b931d0d9a04eda` |
| sha256 | `a1f07f3980407345617f2a8892c8d7296feaf7422d8d713fe84b3bec5da03f22` |

<!-- ASSET file=models/kea_astra.glb md5=fe502de29c851dbfd0b931d0d9a04eda attrib=required author="Macauley.B" title="Rockatoo character" licence="CC-BY-4.0" -->

**THE VERIFICATION CHAIN IS WEAKER AGAIN THAN THE SKETCHFAB ONE, WHICH WAS ALREADY THE WEAK ONE.**
For Poly Haven the md5 comes from the publisher's API and proves the download. For `rockatoo.glb`
there is no publisher checksum, so the hash is of the file as received and proves only that nothing
in this tree altered it since. For this file there is not even a publisher: it is the output of a
generative model, delivered once. The md5 above is of the bytes as they arrived on 2026-09-05 and
detects any later alteration. Nothing upstream can be re-derived or re-requested to check it
against. Recorded, not papered over.

**THE PAINT'S PROVENANCE IS AN OPEN QUESTION AND IS WRITTEN DOWN AS ONE.** The two texture maps were
machine-generated by GPT-6 Astra working from the reference plates Eric collected — photographs of
real kea, of assorted and mostly unrecorded origin, gathered as a look board and never as a licence
tier. Whether a model's output painted from reference photographs inherits anything from those
photographs is **not a settled question**, legally or otherwise, and this project is in no position
to settle it. What can be said precisely is what is true:

- the *geometry and skeleton* lineage is clean and CC-BY, and is credited;
- the *paint* is a model's output, conditioned on plates whose own licences were never established
  because the plates were never intended to ship;
- no plate pixel is shipped — the maps are painted, not composited from the photographs — but that
  is an argument about degree, not a clearance.

**WHAT THAT MEANS FOR SHIPPING, STATED PLAINLY:** this is fine for evaluation, for the side-by-side
strip, and for anything internal. If this texture ever ships in a build that goes out, the plates
behind it need their own provenance established first, or the paint needs regenerating from
material that has it. That is Eric's call, and it is a call, not a formality. Nothing in this piece
makes it for him.
...
### THE BAKED MAPS — `tex/kea_astra_baked_*`  (REPLAT P5F Path A, 2026-09-05)

**THESE ARE THE HARVEST.** Astra's atlas could not be applied to our mesh directly — the two UV
layouts disagree, measured and recorded below — so its painted surface was transferred onto OUR UV
layout by proximity and written out as two new maps. Our geometry is untouched; only the paint
moved. They are a derivative of `kea_astra.glb`, which is a derivative of `rockatoo.glb`, so
CC-BY 4.0 and Macauley.B's credit carry down unchanged, and so does the unsettled-provenance note
on the row above — **the paint in these files is the same machine-generated paint.**

| field | value |
|---|---|
| derived from | `models/kea_astra.glb` -> `models/rockatoo.glb` (Macauley.B, CC-BY 4.0) |
| licence | **CC-BY 4.0, unchanged** |
| made by | proximity bake, `gauntlet/verify/p5f_bake.js`, run against `models/kea_bill.glb`'s UV layout |
| changes | Astra's `Kea_BaseColour` and `Kea_Feather_Normal` resampled from its mesh onto ours: per texel, the point of our surface owning it -> Astra's space -> nearest point on Astra's surface with normal agreement -> Astra's UV there. 12 s per map. |

| file | bytes | md5 |
|---|---|---|
| `tex/kea_astra_baked_basecolour_2048.png` | 4652471 | `c79e2f1871a400c4e929d34e8ac82a47` |
| `tex/kea_astra_baked_normal_2048.png` | 3886116 | `fddb4a36340cb5f54dd6f9253c4344da` |

<!-- ASSET file=tex/kea_astra_baked_basecolour_2048.png md5=c79e2f1871a400c4e929d34e8ac82a47 attrib=required author="Macauley.B" title="Rockatoo character" licence="CC-BY-4.0" -->
<!-- ASSET file=tex/kea_astra_baked_normal_2048.png md5=fddb4a36340cb5f54dd6f9253c4344da attrib=required author="Macauley.B" title="Rockatoo character" licence="CC-BY-4.0" -->

**THE COMPATIBILITY TEST THAT SENT THIS DOWN THE BAKE ROUTE, because P5F.md required the question
be answered by measurement and not by eye.** Both files descend from `rockatoo.glb`, so the UVs
might have survived; they did not. Each vertex was assigned to the skinned region that dominates it
(bone names normalise across the two files by a `cockatoo_`/`kea_` prefix swap — the suffix indices
are identical), and Astra's own baseColour was then sampled at both UV sets:

| pairing | mean per-region colour distance from Astra's own paint |
|---|---|
| **the control** — rockatoo's mesh vs ours, on rockatoo's atlas | **0.4** |
| Astra's atlas applied straight to our mesh (Path A step 1) | **72.3** |
| Astra's paint BAKED onto our layout (Path A fallback) | **3.5** |

**THE CONTROL IS THE HALF THAT MAKES THE OTHER TWO NUMBERS MEAN ANYTHING.** Our mesh IS rockatoo
with vertices deleted, so its UVs are known-compatible with rockatoo's atlas — and the instrument
reads 0.4 there. That is its noise floor. 72.3 is therefore a real mismatch and not a tuning gap,
and 3.5 is a real transfer. Region by region, the straight swap put **orange underwing on the
humerus and metacarpus**, brown body feather on the slate bill, and dropped 12% of our vertices
onto texels below the alpha cutoff. It would have rendered as a plausible-looking bird with a
smeared atlas, which is the exact failure P5F.md section 5 said to report rather than produce.

**WHERE THE BAKE IS WEAKEST, stated rather than averaged away.** Head 18.3 and lower mandible 20.0,
against the 3.5 mean. Both are where our geometry has moved furthest from Astra's: P5d removed the
crest from our crown and P5e lengthened our bill by 1.6x, so proximity has the least to work with
exactly there. The visible consequence is in the strip — Astra's painted eye does not land where
P5e's geometry eye-ring sits, so with both on the bird has two eyes a side. That is a judged call
and it is put in the strip rather than settled here.

**WHY 2048 AND NOT 4096.** Astra's source is 4096x2048 for a bird whose atlas is square, so half of
its width is a layout we do not share. 2048 square is ~0.15 mm per texel on a 0.5 m bird — finer
than the 2 mm the ground materials get, which is right, because this is the one surface the camera
is ever 300 mm from. 4096 would quadruple 8.5 MB to buy detail below the width of a barb.

**SIZE, AND THE DECISION LICENCES.md ALREADY SAID TO WATCH FOR.** These two files are 8.5 MB, taking
the asset tier from 21 MB to ~30 MB, and `kea_astra.glb` itself is another 19 MB. The note under the
P3 tier said "if the asset tier grows past a few tens of MB, that is the moment to reach for git-lfs
or a fetch-on-build step" and named **P6 as the piece that should arrive with that decision already
made**. P5F is the piece that makes it unavoidable rather than theoretical: the tier is now ~49 MB
with the source GLB counted, and P6 adds hero props on top. The decision is still P6's, but it can
no longer be deferred past it.

**`kea_astra.glb` IS A SOURCE ARTIFACT, NOT A RUNTIME ONE.** Nothing loads it at play time — the
game loads our 2.2 MB `kea_bill.glb` and these two maps. It stays in the tree because the bake must
be re-runnable and re-checkable, which is the same reason `rockatoo.glb` stays.

...
C.section('THE CHAPTER TRAVEL BEAT — TODO 34');
{ const T=X.TRAVEL;
  X.startGame(1); tick(8); park();
  const CH=G.chapters.slice();
  /* FLAKES law 1, and it caught this piece: done() writes the save and startGame hydrates it
     straight back, so a run staged after the book was cleared comes up already cleared and the
     page turn under test cannot fire at all. Every stage below goes through here. */
  function freshBook(){ X.startGame(1); tick(8); park();
    for(const m of G.missions){ m.done=false; if(m.need!==undefined)m.n=0; }
    G.chapIdx=0; X.STARS.open(CH[0]); T.end('staging'); tick(2);
    return G.missions.filter(m=>m.done).length; }
  ok(freshBook()===0,'the book can be handed back empty for staging');

  // 1. THE TABLE COVERS THE BOOK, EXACTLY. A named table is only safe while something asserts it
  //    against the book it is keyed to, so: every chapter has an anchor and every anchor is a
  //    chapter. An orphan key is a typo that would otherwise go unnoticed until a beat did nothing.
  const KEYS_=Object.keys(T.ANCHOR);
  ok(CH.length===8,'the book is eight pages long ('+CH.length+')');
  { const missing=CH.filter(a=>!T.ANCHOR[a]);
    ok(missing.length===0,'every chapter has an anchor (missing: '+(missing.join(', ')||'none')+')'); }
  { const orphan=KEYS_.filter(a=>CH.indexOf(a)<0);
    ok(orphan.length===0,'and every anchor is a chapter (orphans: '+(orphan.join(', ')||'none')+')'); }

  // 2. EVERY ANCHOR IS WHERE ITS AREA ACTUALLY IS, measured against landmarks taken from the world
  //    rather than restated here (FLAKES law 10). The sources are deliberately independent of the
  //    table: the CASEFILES crime sites carry a mission id, G handles carry the pen, the campervan
  //    and the nest, and the ski shed and DOC sign are colliders at their build sites.
  const LAND={};
  const put=(area,name,x,z)=>{ (LAND[area]=LAND[area]||[]).push({name,x,z}); };
  for(const cf of X.CASEFILES){ const m=G.missions.find(mm=>mm.id===cf.id);
    if(m&&m.area&&cf.at)put(m.area,'casefile '+cf.id,cf.at.x,cf.at.z); }
  for(const h of (G.hints||[])){ const m=G.missions.find(mm=>mm.id===h.mid);
    if(m&&m.area)put(m.area,'hint '+h.mid,h.x,h.z); }
  { const hut=G.colliders.find(c=>c.kind==='roof'&&c.hut); if(hut)put('THE HUT','the hut roof',hut.x,hut.z); }
  { const shed=G.colliders.find(c=>c.kind==='box'&&Math.abs(c.x+40)<0.01&&Math.abs(c.z+40)<0.01);
    if(shed)put('THE SKI FIELD','the rope-tow shed',shed.x,shed.z); }
  { const sign=G.colliders.find(c=>c.kind==='box'&&Math.abs(c.x-44)<0.01&&Math.abs(c.z+40)<0.01);
    if(sign)put('THE TRAILHEAD','the DOC sign',sign.x,sign.z); }
  if(G.pen)put('THE PADDOCK & NEST','the sheep pen',G.pen.x,G.pen.z);
  if(G.nestPos)put('THE PADDOCK & NEST','the nest',G.nestPos.x,G.nestPos.z);
  if(G.vanTop)put('TOGETHER','the campervan roof',G.vanTop.x,G.vanTop.z);
  const NEAR=8;
  for(const a of CH){ const an=T.ANCHOR[a]||{x:1e6,z:1e6}, ls=LAND[a]||[];
    ok(ls.length>0,'['+a+'] has at least one landmark of its own in the world ('+ls.length+')');
    let best=null;
    for(const l of ls){ const d=Math.hypot(an.x-l.x,an.z-l.z); if(!best||d<best.d)best={d,name:l.name}; }
    ok(!!best&&best.d<=NEAR,'['+a+'] anchor sits '+(best?best.d.toFixed(2):'?')+'u from '+
       (best?best.name:'nothing')+' (<= '+NEAR+')');
    ok(Math.abs(an.x)<=52&&Math.abs(an.z)<=52,'['+a+'] anchor is on the map ('+an.x+','+an.z+')'); }
  { let worst=1e9, pair='';
    for(let i=0;i<CH.length;i++)for(let j=i+1;j<CH.length;j++){
      const p=T.ANCHOR[CH[i]], q=T.ANCHOR[CH[j]]; if(!p||!q)continue;
      const d=Math.hypot(p.x-q.x,p.z-q.z); if(d<worst){worst=d;pair=CH[i]+'/'+CH[j];} }
    ok(worst>6,'no two chapters send the camera to the same place (closest '+worst.toFixed(1)+'u, '+pair+')'); }

  // 3. THE WEIGHT CURVE IS THE WHOLE ANIMATION, so it is asserted as a curve: nought at both ends,
  //    up, flat, down, and continuous. A discontinuity here is a camera that jumps.
  const SPAN=T.SPAN;
  ok(Math.abs(SPAN-(T.BEAT.out+T.BEAT.hold+T.BEAT.back))<1e-9,'the span is the sum of its phases ('+SPAN.toFixed(2)+'s)');
  ok(T.weight(0)===0&&T.weight(SPAN)===0,'the beat starts and ends on the follow camera exactly');
  ok(T.weight(T.BEAT.out)===1&&T.weight(T.BEAT.out+T.BEAT.hold-1e-6)===1,'and holds the far camera outright across the hold');
  { let up=true,down=true,jump=0,prev=T.weight(0);
    for(let t=0;t<=SPAN+0.001;t+=0.001){ const w=T.weight(t);
      jump=Math.max(jump,Math.abs(w-prev));
      if(t<=T.BEAT.out&&w<prev-1e-12)up=false;
      if(t>=T.BEAT.out+T.BEAT.hold&&w>prev+1e-12)down=false;
      prev=w; }
    ok(up,'the flight out never backtracks');
    ok(down,'the flight home never backtracks');
    ok(jump<0.01,'and the curve is continuous at 1ms steps (worst step '+jump.toFixed(4)+')'); }
  ok(T.phase(0)==='out'&&T.phase(T.BEAT.out)==='hold'&&T.phase(T.BEAT.out+T.BEAT.hold)==='back',
     'the phases name themselves at their own boundaries');

  // 4. THE AIM IS COMPUTED, NOT GUESSED, and it clears the country at every anchor — the ski field
  //    is the one that matters, since its anchor is the shed and the shed roof is at 2.0.
  for(const a of CH){ const an=T.ANCHOR[a]; if(!an)continue;
    const aim=T.aim({x:an.x,z:an.z,y:T.BEAT.high});
    const gy=X.groundHeightAt(an.x,an.z,T.BEAT.high);
    ok(aim.y>=gy+T.BEAT.rise-1e-9,'['+a+'] the beat camera clears the ground under it ('+
       aim.y.toFixed(2)+' over '+gy.toFixed(2)+')');
    ok(aim.lx===an.x&&aim.lz===an.z,'['+a+'] and it looks at the anchor itself');
    ok(Math.abs(aim.z-(an.z+T.BEAT.standoff))<1e-9,'['+a+'] from one named standoff, never overhead'); }

  // 5. A PAGE TURN OPENS A BEAT ON THE PAGE IT TURNED TO — driven through the real done list, since
  //    the turn is the while loop in missionDone and nothing else may be trusted to fire it. Every
  //    turn in the book is driven, not just the first: seven turns, seven beats, seven right areas.
  ok(G.travel===null,'a fresh run opens with no beat running');
  freshBook();
  let turns=0, wrong=[];
  for(let i=1;i<CH.length;i++){
    G.chapIdx=i-1; T.end('staging'); tick(2);
    const rows=X.STARS.rows(CH[i-1]);
    if(!rows.length){ continue; }
    for(let q=0;q<rows.length-1;q++)rows[q].done=true;
    X.done(rows[rows.length-1].id);
    const b=G.travel;
    if(G.chapIdx===i&&b&&b.area===CH[i])turns++; else wrong.push(CH[i-1]+'->'+(b?b.area:'no beat'));
    ok(!b||(b.t===0&&b.w===0&&b.phase==='out'&&b.card===CH[i]&&b.cardOn===false),
       '['+CH[i]+'] the beat opens at rest, on the follow camera, card not yet up');
    T.end('staging'); }
  ok(turns===CH.length-1,'every page turn in the book opens its own beat ('+turns+' of '+(CH.length-1)+
     (wrong.length?'; wrong: '+wrong.join(', '):'')+')');

  // 6. THE TIMER RUNS THE BEAT AND HANDS THE CAMERA BACK. Ticked frame by frame with no input, the
  //    beat must pass through all three phases, reach the far camera outright, then clear itself.
  freshBook();
  { const k=kq(); k.x=20; k.z=-30; k.y=X.groundHeightAt(20,-30,3); k.vy=0; k.grounded=true; tick(4);
    const p0={x:k.x,y:k.y,z:k.z};
    G.chapIdx=0; const rows=X.STARS.rows(CH[0]);
    for(let q=0;q<rows.length-1;q++)rows[q].done=true;
    X.done(rows[rows.length-1].id);
    ok(!!G.travel&&G.travel.area===CH[1],'the beat is up, aimed at '+(G.travel?G.travel.area:'nothing'));
    const seen={}, cards=new Set(); let wmax=0, cardOnlyInHold=true, frames=0;
    while(G.travel&&frames<600){ X.update(1/60); frames++;
      const b=G.travel; if(!b)break;
      seen[b.phase]=(seen[b.phase]||0)+1; wmax=Math.max(wmax,b.w);
      if(b.cardOn){ cards.add(b.card); if(b.phase!=='hold')cardOnlyInHold=false; } }
    ok(seen.out>0&&seen.hold>0&&seen.back>0,'it flew out, held, and came home ('+JSON.stringify(seen)+')');
    ok(wmax===1,'and it reached the far camera outright on the way ('+wmax+')');
    ok(G.travel===null,'the timer handed the camera back ('+frames+' frames, span '+SPAN.toFixed(2)+'s)');
    ok(!!G.travelLast&&G.travelLast.ended==='expiry','and it recorded why it ended ('+
       (G.travelLast?G.travelLast.ended:'nothing')+')');
    ok(G.travelLast&&G.travelLast.w===0&&G.travelLast.cardOn===false,'with the weight and the card put away');
    ok(cards.size===1&&cards.has(CH[1]),'the title card named the new page and nothing else ('+
       [...cards].join(', ')+')');
    ok(cardOnlyInHold,'and it was only up while the camera was actually there');
    ok(Math.hypot(k.x-p0.x,k.z-p0.z)<1e-9&&Math.abs(k.y-p0.y)<1e-9,
       'THE BIRD DID NOT MOVE: no teleport, no nudge ('+Math.hypot(k.x-p0.x,k.z-p0.z).toFixed(6)+'u)'); }

  // 7. ANY FRESH INPUT SKIPS IT, AND THE ARM DELAY IS WHAT MAKES THAT POSSIBLE AT ALL. The turn is
  //    caused by a keypress, so the finishing key is still in PRESSED on the beat's first frame; a
  //    beat that skipped on that could never be seen. Three cases, all three staged.
  function openBeat(pageIdx){ T.end('staging'); tick(2);
    G.chapIdx=pageIdx; const rows=X.STARS.rows(CH[pageIdx]);
    for(const m of rows)m.done=false;
    for(let q=0;q<rows.length-1;q++)rows[q].done=true;
    X.done(rows[rows.length-1].id); return G.travel; }
  { freshBook();
    ok(!!openBeat(0),'staged a beat to interrupt');
    X.press('KeyW'); X.update(1/60);
    ok(!!G.travel&&G.travel.t<T.BEAT.arm,'a press inside the arm delay does not eat the beat (t '+
       (G.travel?G.travel.t.toFixed(3):'gone')+' < '+T.BEAT.arm+')');
    X.release('KeyW');
    while(G.travel&&G.travel.t<T.BEAT.arm)X.update(1/60);
    ok(!!G.travel,'the beat is still up once the skip arms');
    X.press('KeyE'); X.update(1/60); X.release('KeyE');
    ok(G.travel===null&&G.travelLast&&G.travelLast.ended==='skip',
       'and a fresh press after that ends it ('+(G.travelLast?G.travelLast.ended:'still running')+')'); }
  { freshBook();
    X.press('KeyW'); tick(3);                                   // walking BEFORE the turn: not a skip
    ok(!!openBeat(0),'staged a beat while a key was already held');
    ok(!!G.travel&&G.travel.held0&&G.travel.held0.has('KeyW'),'the beat remembers what was already down');
    let f=0; while(G.travel&&f<600){ X.update(1/60); f++; }
    X.release('KeyW');
    ok(!!G.travelLast&&G.travelLast.ended==='expiry',
       'a key held from before the turn is not new input, so the beat ran its course ('+
       (G.travelLast?G.travelLast.ended:'?')+')');
    X.press('KeyW'); tick(3);
    ok(!!openBeat(1),'staged another, still holding');
    while(G.travel&&G.travel.t<T.BEAT.arm)X.update(1/60);
    X.press('KeyA'); X.update(1/60); X.release('KeyA'); X.release('KeyW');
    ok(G.travel===null&&!!G.travelLast&&G.travelLast.ended==='skip','but a SECOND key going down is ('+
       (G.travelLast?G.travelLast.ended:'no beat was ever opened')+')'); }

  // 8. THE BEAT REFUSES TO OPEN WHERE IT HAS NO BUSINESS. Colossal has no chapters at all, the title
  //    screen has no bird to leave, and an area with no anchor must fail loudly rather than fly to
  //    the origin — which is what the orphan assertion in 1 is guarding against on the other side.
  { freshBook();
    ok(T.start('THE MOON')===null,'an area with no anchor opens nothing');
    ok(T.start(null)===null&&T.start('')===null,'and neither does no area at all');
    G.colossal=true;  ok(T.start(CH[1])===null,'colossal mode opens nothing (it has no chapters)'); G.colossal=false;
    G.running=false;  ok(T.start(CH[1])===null,'the title screen opens nothing');            G.running=true;
    G.won=true;       ok(T.start(CH[1])===null,'and a finished run opens nothing');          G.won=false;
    ok(!!T.start(CH[1]),'while a live run opens one'); T.end('staging'); }

  // 9. A RESTART TAKES THE BEAT WITH IT (FLAKES law 1: the world persists across startGame, so
  //    anything left running would ride into the next run and hold the camera off the new bird).
  { freshBook(); ok(!!openBeat(0),'a beat is running when the restart comes');
    X.startGame(1); tick(4);
